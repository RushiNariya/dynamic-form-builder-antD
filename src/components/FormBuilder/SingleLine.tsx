...
function SingleLine() {
  ...
  const initialData = useMemo(() => {
    return SIDEBAR_ITEMS.find((item) => item.fieldName === FormFieldTypes.SINGLE_LINE);
  }, [SIDEBAR_ITEMS]);

  const validationData = useMemo(() => {
    return validationOptions.filter((item) => {
      return item.fieldName.includes(FormFieldTypes.SINGLE_LINE);
    });
  }, [validationOptions]);
  ...
  const handleOneNestedChange = (key: string, value: string) => {
    setLocalData((data: SidebarItemType) =>
    ...
    <div className="px-3 py-2">
      <form onSubmit={submitHandler}>
        <div className="flex justify-between items-center">
          <h3 className="font-semibold">Single Line </h3>
